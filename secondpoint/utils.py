...
model.load_state_dict(torch.load('utility_model.pt', map_location="cpu"))
model.eval()
...
def predict_single(text, model=model, temp=5, inverse_category_list=new_category):
    ...
    with torch.no_grad():
        ...
        if temp == 0 or actual_length == 0:
            return "Insufficient Product Description: Please check the language used, the size of the text, and its meaning.", []
